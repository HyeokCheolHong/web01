...
<title> </title>
 ...
 <script type="text/javascript">
   function check(){
	   let name = $.trim($('#writer').val());//trim()함수는 양쪽 공백을 제거, val()함수는 입력필드 입력값을 가져옴
	   if(name.length == 0){//length속성은 문자 길이를 반환, 첫문자는 1부터 시작
		   alert('글쓴이를 입력하세요!');
	       $('#writer').val('');//입력필드 초기화
	   ...
	   if($title == ''){
		   alert('글제목을 입력하세요!');
		   $('#title').val('').focus();//입력필드 초기화와 포커스 이동을 메서드 체이닝법으로 처리
		   return false;
	   }
